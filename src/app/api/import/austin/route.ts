...
import austinListings from "../../../../../data/austin_listings.json";

interface AustinBuilding {
  name: string;
  address: string;
  neighborhood: string;
  phone?: string;
...
export async function POST() {
  try {
    ...
    // Get Austin city
    let { data: austinCity } = await supabase
      .from("cities")
      .select("id")
      .eq("slug", "austin")
      .single();
    ...
    if (!austinCity) {
      ...
      if (error) {
        return NextResponse.json({ error: `Failed to create Austin: ${error.message}` }, { status: 500 });
      }
      austinCity = newCity;
    }
    ...
    for (const building of (austinListings as { buildings: AustinBuilding[] }).buildings) {
      try {
        ...
        const zip = zipMatch ? zipMatch[0] : "78701"; // Default Austin zip
    ...
    return NextResponse.json({
      success: true,
      message: "Austin buildings imported successfully",
      results,
    });
  } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    message: "POST to this endpoint to import Austin buildings",
    buildings_count: (austinListings as { buildings: unknown[] }).buildings.length,
  });
}
